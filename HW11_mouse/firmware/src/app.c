...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            int x=((int)data[4]*8)/16000;
            int y=-((int)data[5]*5)/16000;
            
            if(!is_pressed&&PUSH_BUTTON==0)
            {
                is_pressed=1;
                appData.mouseButton[0]=MOUSE_BUTTON_STATE_PRESSED;
            }
            else if(is_pressed&&PUSH_BUTTON==1)
            {
                is_pressed=0;
            ...
            if(inc==3)
            {
                ...
                appData.xCoordinate = (int8_t) y;
                appData.yCoordinate = (int8_t) x;
            }
            else
            {
                //appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
